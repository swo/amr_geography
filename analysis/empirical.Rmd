...
# marketscan unit data
mud = marketscan %>%
  filter(bugdrug == 'Ec/q') %>%
  select(unit) %>%
  distinct() %>%
  left_join(state_data, by = c('unit' = 'state'))

# marketscan distances
msd = unit_distances(mud)

# one random sample & random zone
rs = sample1(length(mud$unit), 5)
rz = random_zones(msd, 5)

m %>%
  as_tibble() %>%
  left_join(data_frame(region = tolower(names(rz)), zone = rz), by = 'region') %>%
  left_join(data_frame(region = mud$unit, sam = rs, by = 'region')) %>%
  gather('key', 'value', rz, rs) %>%
  ggplot(aes(x = long, y = lat)) +
  geom_polygon(aes(fill = factor(agg)))

data_frame(state = tolower(names(rz)), agg = rz) %>%
  ggplot(aes(map_id = state)) +
  geom_map(aes(fill = factor(agg)), map = m) +
...
maup_sim = function(useres, unit_data, ks, n_iterations = 10, extra_groups = NULL) {
  ...
  bugdrugs = unique(useres$bugdrug)
