...
region_boot_f = function(df, group_chr, n_trials = 1e2, alpha = 0.05, pred_n = 1e4) {
  ...
  results = data_frame(i = 1:n_trials) %>%
    mutate(
      ...
      unit_data = map(df, ~ sample_n(., size = nrow(.), replace = TRUE)),
...
region_models = region_data %>%
  mutate(
    ...
    boot_cil = map_dbl(boot_results, ~ .$slope_cil),
    boot_ciu = map_dbl(boot_results, ~ .$slope_ciu),
...
region_models %>%
  ...
  select(dataset, bugdrug, group_type, slope) %>%
  arrange(dataset, bugdrug, group_type) %>%
  kable()

region_models %>%
  ggplot(aes(group_type, slope, ymin = slope_cil, ymax = slope_ciu)) +
...
marketscan_regional_plot = ggplot(data = NULL, aes(use, color = group_fct, fill = group_fct)) +
  ...
  geom_point(data = marketscan_region_plot_data, aes(y = res)) +
  geom_smooth(data = marketscan_region_plot_data, aes(y = res), method = 'lm', se = FALSE) +
...
other_regional_plot = ggplot(data = NULL, aes(use, color = group_fct, fill = group_fct)) +
  facet_wrap(~ label, scales = 'free') +
  geom_ribbon(data = other_region_pred_data, aes(ymin = ymin, ymax = ymax), alpha = 0.25) +
  scale_fill_manual(values = my_palette) +
  scale_color_manual(values = my_palette) +
  geom_point(data = other_region_plot_data, aes(y = res)) +
  geom_smooth(data = other_region_plot_data, aes(y = res), method = 'lm', se = FALSE) +
  xlab('antibiotic use (annual treatments per capita)') +
  scale_y_continuous(
    'antibiotic resistance (% of isolates)',
    labels = scales::percent_format(accuracy = 1, suffix = '')
...
# compute indices, models, and predictions
distance_boot_f = function(df, n_trials = 1e2, alpha = 0.05, pred_n = 1e4) {
