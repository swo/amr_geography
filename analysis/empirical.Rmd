...
regression_p = function(bd, n_iter = 99, method = bisquare, max_dist = Inf) {
  vals = function(permute) {
    ...
    arru = method(arru ~ du + du:d + d_density + d_income + d_temp, data = dfd) %>% coef2()
    loru = method(loru ~ du + du:d + d_density + d_income + d_temp, data = dfd) %>% coef2()
    # unadjusted models
    arru_ua = method(arru ~ du + du:d, data = dfd) %>% coef2()
    loru_ua = method(loru ~ du + du:d, data = dfd) %>% coef2()
...
distance_fit_results %>%
  select(dataset, bugdrug, arru_p, loru_p, arru_p_ua, loru_p_ua) %>%
  kable(caption = 'Permutation p values')

distance_fit_results %>%
  rename(model = model_arru) %>%
  mutate(ci = map(model, ~ as_tibble(confint(.), rownames = 'term'))) %>%
  select(dataset, bugdrug, ci) %>%
  unnest() %>%
  filter(term %in% c('du', 'du:d')) %>%
  mutate(`2.5%` = if_else(term == 'du:d', `2.5%` * 1e4, `2.5%`)) %>%
  mutate(`97.5%` = if_else(term == 'du:d', `97.5%` * 1e4, `97.5%`)) %>%
  kable(caption = 'ARRU model term CIs')

distance_fit_results %>%
  rename(model = model_loru) %>%
  mutate(ci = map(model, ~ as_tibble(confint(.), rownames = 'term'))) %>%
  select(dataset, bugdrug, ci) %>%
  unnest() %>%
  filter(term %in% c('du', 'du:d')) %>%
  mutate(`2.5%` = if_else(term == 'du:d', `2.5%` * 1e4, `2.5%`)) %>%
  mutate(`97.5%` = if_else(term == 'du:d', `97.5%` * 1e4, `97.5%`)) %>%
  kable(caption = 'LORU model term CIs')

distance_fit_results %>%
  mutate(arru = map_dbl(model_arru, coef2),
         loru = map_dbl(model_loru, coef2)) %>%
  select(dataset, bugdrug, arru, loru) %>%
  mutate_if(is.numeric, ~ round(. * 1e4, 2)) %>%
  kable(caption = 'Point estimates for ratio')
```
