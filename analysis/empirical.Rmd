...
marketscan = marketscan_use %>%
  ...
  select(
    unit_id, unit, bugdrug, use, n_resistant, n_isolates,
...
europe = read_tsv('../data/ecdc/data.tsv') %>%
  ...
  select(
    unit_id, unit, bugdrug, use, n_resistant = n_ns, n_isolates,
...
sample_nmin = function(n, k, n_min = 2) {
  # ensure it's possible with these values
  stopifnot(n_min * k <= n)
  
  # take each ID, the 1:k, n_min times
  rep(1:k, each = n_min) %>%
    # then, take the rest of the n at random from 1:k
    c(sample(1:k, n - length(.), replace = TRUE)) %>%
...
inv_logodds = function(q) 1 / (exp(-q) + 1)

aggregate = function(df, group_chr) {
  stopifnot(class(group_chr) == 'character')
  group = as.symbol(group_chr)
  
  group_by_at(df, c(group_chr, 'bugdrug')) %>%
    summarize(
      group = unique(!!group),
...
shuffle_slopes = function(df, group_chr, n_trials = 99) {
  map_dbl(1:n_trials, function(i) {
    df %>%
      mutate_at(group_chr, sample) %>%
      aggregate(group_chr) %>%
      glm(cbind(n_resistant, n_susceptible) ~ use, family = 'binomial', data = .) %>%
      { coef(.)['use'] }
  })
}

shuffle_slopes(marketscan, 'us_region')

m1 = map_dfr(c('unit', 'us_region', 'us_division'), ~ aggregate(marketscan, .)) %>%
  nest(-bugdrug, -group_type) %>%
  mutate(
    # run the models for each aggregate type
    model = map(data, ~ glm(cbind(n_resistant, n_susceptible) ~ use, family = 'binomial', data = .)),
    ...
    pred_x = map(data, ~ seq(from = min(.$use), to = max(.$use), length.out = 100)),
    pred = map2(
      model, pred_x,
      ~ predict(.x, newdata = data_frame(use = .y), type = 'response', se.fit = TRUE)
    ),
    pred_data = map2(pred_x, pred, ~ data_frame(
      use = .x,
      res = .y$fit,
      ymax = .y$fit + 1.96 * .y$se.fit,
      ymin = .y$fit - 1.96 * .y$se.fit
    ))
  ) %>%
  arrange(bugdrug)
...
base_slopes = m1 %>%
  filter(group_type == 'unit') %>%
  select(bugdrug, slope)

null_slopes = m1 %>%
  select(bugdrug, group_type) %>%
  filter(group_type != 'unit') %>%
  left_join(base_slopes, by = 'bugdrug') %>%
  rename(base_slope = slope) %>%
  mutate(
    null_slopes = map2(bugdrug, group_type, ~ shuffle_slopes(filter(marketscan, bugdrug == .x), .y)),
    p_value = map2_dbl(null_slopes, base_slope, ~ (sum(.x > .y) + 1) / (length(.x) + 1))
  )

# combine actual data, "prediction" data, and p-values
mm = m1 %>%
  left_join(null_slopes, by = c('bugdrug', 'group_type')) %>%
  select(
    bugdrug, group_type,
    data, model, slope, slope_cil, slope_ciu, p_value,
    pred_data
  )

# get the data
m2 = m1 %>%
  select(bugdrug, group_type, data) %>%
  unnest()

# "data" from the predictions
m3 = m1 %>%
  select(bugdrug, group_type, pred_data) %>%
  unnest()

ggplot(data = NULL, aes(use, res)) +
  facet_wrap(~ bugdrug) +
  geom_ribbon(data = m3, aes(fill = group_type, ymin = ymin, ymax = ymax), color = NA) +
  geom_point(data = m2, aes(shape = group_type)) +
  geom_line(data = m3, aes(linetype = group_type))
```

```{r}
# put p-values around all that
```
