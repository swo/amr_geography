...
f = function(bd, n_iter = 1e3) {
  ...
  nls_model = nls(dr ~ A * (1 + R * dist) * du + C,
                  start = list(A = obs_slope, R = obs_ratio, C = 0.0),
                  data = x)
...
g = function(dataset, lst) {
  data_frame(
    ...
    bugdrug = names(lst),
    results = map(lst, f),
    obs_ratio = map_dbl(results, ~ 1e3 * .$obs_ratio),
    p = map_dbl(results, ~ .$p)
  )
}
...
res %>%
  mutate(sig = p.adjust(p, 'BH') < 0.05,
         #nlo = map_dbl(results, ~ .$nls_model %>% coef %>% { 1e3 * .['R'] }),
         nlp = map_dbl(results, ~ .$nls_model %>% summary %>% coef %>% { .['R', 'Pr(>|t|)']}),
         nlsig = p.adjust(nlp, 'BH') < 0.05,
         nllci = map_dbl(results, ~ .$nls_model %>% confint.default %>% { 1e3 * .['R', 1] }),
         nluci = map_dbl(results, ~ .$nls_model %>% confint.default %>% { 1e3 * .['R', 2] })) %>%
  select(-results) %>%
...
bs_p = function(bs, x0) sum(abs(bs$t - x0) > abs(bs$t0 - x0)) / (bs$R + 1)
...
g = function(dataset, lst) {
  data_frame(
    ...
    results = map(lst, f),
    obs_ratio = map_dbl(results, ~ 1e3 * .$obs_ratio),
    lci = map_dbl(results, ~ 1e3 * .$bs_lci),
    uci = map_dbl(results, ~ 1e3 * .$bs_uci),
    p = map_dbl(results, ~ bs_p(.$bs, 0))
  )
}
...
bs_res %>%
  select(-results) %>%
  kable(caption = 'Bootstrap CIs. In per 1,000 km.')
```
