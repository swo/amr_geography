...
marketscan_canonical_data %>%
  ...
  mutate(
    model = map(data, ~ lm(pct_res ~ did, data = .)),
    slope = map_dbl(model, ~ coef(.)['did']),
    slope_cil = map_dbl(model, ~ confint(.)['did', 1]),
    slope_ciu = map_dbl(model, ~ confint(.)['did', 2])
  ) %>%
...
marketscan_state_slopes = marketscan_canonical_data %>%
  ...
  mutate(model = map(data, ~ lm(pct_res ~ did, data = .)),
         state_slope = map_dbl(model, ~ coef(.)['did'])) %>%
...
marketscan_maup %>%
  ...
  select(bugdrug, k, group_type, state_slope, median_super, iqr_super, wilcox_p, p2, fdr) %>%
...
marketscan_maup %>%
  ...
  geom_vline(data = ss, aes(xintercept = state_slope), color = 'blue', linetype = 2) +
