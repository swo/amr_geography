...
marketscan_group = function(bugdrug, k, group_type) {
  bugdrug_data = marketscan_maup_data[[bugdrug]]
  
  switch(group_type,
         'sample' = sample1(bugdrug_data$n_units, k),
         'zone' = random_zones(marketscan_maup_data[[bugdrug]]$unit_distances, k),
         'region' = renumber(state.region[bugdrug_data$state_id]),
         'division' = renumber(state.division[bugdrug_data$state_id]),
         'state' = bugdrug_data$state_id
  )
}
...
marketscan_maup = try_load('results/marketscan_maup.rds', {
  ...
  crossing(
    ...
    bind_rows(canonical) %>%
    mutate(
      group = pmap(list(bugdrug, k1, group_type), marketscan_group),
