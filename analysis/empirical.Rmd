...
random_groups = function(states, k) {
  ...
  zone_centers = state_centers %>%
    filter(state %in% states) %>%
    sample_n(k) %>%
    select(zone = state, center_long = long, center_lat = lat)
    
  zones = crossing(state_centers, zone_centers) %>%
    mutate(dist = sqrt((long - center_long) ** 2 + (lat - center_lat) ** 2)) %>%
    group_by(state) %>%
  ...
  data_frame(state = states,
             sample = as.character(sample1(length(states), k))) %>%
