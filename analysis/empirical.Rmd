...
adjacency_permute_f = function(df, model_f, stat_f, n_trials = 1e2 - 1, alpha = 0.05) {
...
median_drdu_by_adjacency <- function(df) {
  df %>%
    group_by(adjacent) %>%
    summarize(y = median(dr_du)) %>%
    with(y[adjacent == 1] - y[adjacent == 0])
}

adjacency_results <- adjacency_analysis(
  median_drdu_by_adjacency,
  identity
)
```
...
adjacency_results2 %>%
  mutate(
    ar = map(results, function(r) {
      r$base_data %>%
        group_by(adjacent) %>%
        summarize(y = median(dr_du))
    })
  ) %>%
  select(dataset, bugdrug, ar) %>%
  unnest() %>%
  mutate(adjacent = recode(adjacent, `0` = 'not', `1` = 'adj')) %>%
  spread(adjacent, y)
```
...
commuting_results <- unit_data %>%
  ...
  mutate(
    results = map(data, ~ adjacency_permute_f(., commute_model_f, commute_stat_f, n_trials = 9)),
    results_lor = map(data, ~ adjacency_permute_f(., commute_lor_f, commute_stat_f, n_trials = 9))
  )
