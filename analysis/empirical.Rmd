...
```{r combine_data}
named_bind_rows <- function(..., label = 'data_source') {
...
analysis_f <- function(model_f, coef_f, ratio_f) {
  base_results <- cross_data %>%
    ...
    select(dataset, bugdrug, coef, ratio)
  
  l1o_results <- cross_data %>%
    select(dataset, bugdrug, l1o_cross_data) %>%
    unnest() %>%
    mutate(
  ...
  base_results %>%
    left_join(l1o_results, by = c('dataset', 'bugdrug'))
}
```
...
analysis_f(
  function(df) with(df, {
    list(
      overall_median = median(df$dr_du),
      adjacent_median = median(df$dr_du[df$adjacent]),
      nonadjacent_median = median(df$dr_du[!df$adjacent])
    )
  }),
  function(model) with(model, { adjacent_median - nonadjacent_median }),
  function(model) with(model, { (adjacent_median - nonadjacent_median) / overall_median })
) %>%
  mutate(
  ...
  select(dataset, bugdrug, coef, coef_cil, coef_ciu, ratio, ratio_cil, ratio_ciu) %>%
  kable(caption = 'Using adjacency and medians')
```

```{r adjacency_lorru}
analysis_f(
  function(df) rlm(lorr_du ~ adjacent, data = df),
  function(model) coef(model)['adjacentTRUE'],
  function(model) coef(model) %>% { .['adjacentTRUE'] / .['(Intercept)'] }
) %>%
  kable(caption = 'Using LORR /du')
```
...
analysis_f(
  ...
  kable(caption = 'Using adajency with rlm')
```
...
analysis_f(
  ...
  kable(caption = 'Using adjacency with rlm & covariates')
```

```{r commuting_analysis}
analysis_f(
  function(df) rlm(dr_du ~ f_commuting, data = df),
  function(model) coef(model)['f_commuting'] * 0.01,
  function(model) coef(model) %>% { .['f_commuting'] / .['(Intercept)'] * 0.01 }
) %>%
  kable(caption = 'Using commuting fraction')
```
...
```{r adjacency_results, eval = FALSE}
...
```{r adjacency_covariates_analysis, eval = FALSE}
