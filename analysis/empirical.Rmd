...
null_slopes_f = function(df, group_chr, n_trials = 99) {
...
region_models %>%
  select(dataset, bugdrug, group_type, slope, slope_cil, slope_ciu, p_value) %>%
...
marketscan_regional_plot = ggplot(data = NULL, aes(use, res)) +
  ...
  geom_ribbon(data = marketscan_region_pred_data, aes(ymin = ymin, ymax = ymax, fill = group_fct), color = NA, alpha = 0.25) +
  ...
  scale_y_continuous(
    'antibiotic resistance (proportion of isolates)',
...
$$
\Delta \rho \sim \Delta \tau + \Delta \tau \times d + \Delta \mathrm{income} + \ldots
$$
...
null_interactions_f = function(df, n_trials = 99) {
  replicate(n_trials, {
    df %>%
      shuffle_useres %>%
      cross_units %>%
...
distance_results %>%
  select(dataset, bugdrug, coef_d_use, coef_d_use_adjacent, coef_ratio, p_value) %>%
  kable()
```
...
```{r distance_plots}
eu_ecq_distance_plot = distance_results %>%
  filter(dataset == 'europe', bugdrug == 'Ec/q') %$%
  pair_data[[1]] %>%
  mutate(adjacent = adjacent == 1) %>%
  ggplot(aes(d_use, d_resistant)) +
  geom_point(aes(shape = adjacent)) +
  scale_shape_manual(values = c(1, 16)) +
  stat_smooth(aes(linetype = adjacent), method = 'rlm', color = 'black', size = 0.5) +
  scale_linetype_manual(values = c(1, 2)) +
  xlab('Difference in antibiotic use (annual treatments per capita)') +
  ylab('Absolute difference in proportion of isolates resistant') +
  theme_classic() +
  theme(
    legend.position = 'none',
    strip.text = element_text(size = 8),
...
ggsave(
  'fig/eu_ecq_distance_plot.pdf',
  plot = eu_ecq_distance_plot,
