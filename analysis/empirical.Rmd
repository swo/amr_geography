...
group_lm_simulation = function(ur, n_groups, n_iterations) {
  lapply(1:n_iterations, function(i) group_lm(ur, n_groups)) %>%
    bind_rows() %>%
    filter(term == 'piy') %>%
    summarize(n_groups = n_groups,
              mean_slope = mean(estimate),
              std_slope = sd(estimate))
}

list_n_groups = c(4, 9, 15, 20, 25, 30, 35, 40, 44)
#list_n_groups = c(4, 9, 44) 
n_iterations = 100
...
maup_combined %>%
  ...
  geom_point(inherit.aes=FALSE, data=filter(maup_state, term=='piy'), aes(x=44, estimate)) +
  ...
  #geom_errorbar(inherit.aes=FALSE, data=filter(maup_division, term=='piy'), aes(x=9, ymin=conf.low, ymax=conf.high)) +
  geom_point(inherit.aes=FALSE, data=filter(maup_region, term=='piy'), aes(x=4, estimate)) +
  #geom_errorbar(inherit.aes=FALSE, data=filter(maup_region, term=='piy'), aes(x=4, ymin=conf.low, ymax=conf.high))
