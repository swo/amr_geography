...
marketscan_maup %>%
  mutate(within_slopes = map(within_models, function(ms) { map_dbl(ms, function(m) coef(m)['did']) }),
         slope_q = map2(within_slopes, slope, qf)) %>%
  filter(map_lgl(slope_q, ~ is.null(.$error))) %>%
  mutate(slope_q2 = map_dbl(slope_q, ~ .$result)) %>%
  ggplot(aes(factor(k), slope_q2, color = group_type)) +
  facet_wrap(~ bugdrug) +
  geom_boxplot()
```
