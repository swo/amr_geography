...
show_results <- function(df, caption) {
  df %>%
    ...
    select(dataset, coef, coef_cil, coef_ciu, ratio, ratio_cil, ratio_ciu) %>%
...
adjacency_bisquare_results <- analysis_f(
  function(df) bisquare(dr_du ~ adjacent, data = df),
  function(model) coef(model)['adjacentTRUE'],
  function(model) coef(model) %>% { .['adjacentTRUE'] / .['(Intercept)'] }
)

show_results(
  adjacency_bisquare_results,
  "As above, but using Tukey's bisquare."
)
```
...
show_results(
  ...
  'As above, but with Δρ/Δτ ~ Α + X, where X are the other covariates.'
)
```
...
show_results(
  commuting_results,
  'Robust regression on Δρ/Δτ ~ C, where C is the commuting fraction. "Coef" is βC. "Ratio" is βC / μ.'
)
```
