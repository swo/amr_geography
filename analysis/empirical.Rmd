...
other_regional_plot = ggplot(data = NULL, aes(use, res, color = group_fct, fill = group_fct)) +
  ...
  geom_ribbon(data = other_region_pred_data, aes(ymin = ymin, ymax = ymax), alpha = 0.25) +
  scale_fill_manual(values = my_palette) +
  scale_color_manual(values = my_palette) +
...
main_distance_plot = ggplot(data = NULL, aes(d_use, color = adjacent, fill = adjacent)) +
  ...
  geom_ribbon(data = main_distance_pred_data, aes(ymin = ymin, ymax = ymax), alpha = 0.25) + 
...
other_distance_plot = ggplot(data = NULL, aes(d_use, color = adjacent, fill = adjacent)) +
  facet_wrap(~ label, scales = 'free') +
  geom_point(data = other_distance_plot_data, aes(y = d_resistant), shape = 1, size = 0.5) +
  stat_smooth(data = other_distance_plot_data, aes(y = d_resistant), method = 'rlm', se = FALSE) +
  geom_ribbon(data = other_distance_pred_data, aes(ymin = ymin, ymax = ymax), alpha = 0.25) + 
  scale_color_manual(values = c('black', 'red')) +
  scale_fill_manual(values = c('black', 'red')) +
  xlab('Difference in antibiotic use (annual treatments per capita)') +
  ylab('Difference in resistance (log odds ratio)') +
