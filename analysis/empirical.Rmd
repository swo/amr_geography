...
europe = read_tsv('../data/ecdc/data.tsv') %>%
  ...
  select(unit, bugdrug, use = did, n_resistant = n_ns, n_isolates, population)
...
nhsn = read_tsv('../data/nhsn-ims/data.tsv') %>%
  ...
  select(dataset, bug, drug, unit = state, use = rx_1k_year, pct_res, n_isolates)
```
...
maup_table = function(maup, base_level = 'state', ks = c(4, 9)) {
  ...
  maup %>%
    ...
    mutate(wilcox_p = map2_dbl(data, base_slope, ~ wilcox.test(.x - .y, alternative = 'greater')$p.value),
...
nhsn = read_tsv('../data/nhsn-ims/data.tsv') %>%
  ...
  arrange(unit) %>%
  left_join(select(state_data, unit, population), by = 'unit') %>%
  select(unit, bugdrug, use = rx_1k_year, n_resistant, n_isolates, population)
...
```{r}
agg_slope = function(df, group_, permute = FALSE) {
  groupn_ = quo_name(group_)
  
  expected_names = c(groupn_, 'use', 'population', 'n_resistant', 'n_isolates')
  cat(setdiff(expected_names, names(df)))
  stopifnot(all(expected_names %in% names(df)))
  
  permute_f = if (permute) {
    function(x) mutate(x, !!groupn_ := sample(!!group_))
  } else {
    identity
  }
...
agg_slope_p = function(df, group, n_iterations) {
  group_ = enquo(group)
  obs_slope = agg_slope(df, group_, permute = FALSE)
  perm_slopes = replicate(n_iterations, agg_slope(df, group_, permute = TRUE))
...
marketscan %>%
  ...
  nest(-bugdrug) %>%
  mutate(region_p = map_dbl(data, ~ agg_slope_p(., region, 100)),
         division_p = map_dbl(data, ~ agg_slope_p(., division, 100)))
...
europe %>%
  left_join(europe_units) %>%
  nest(-bugdrug) %>%
  mutate(region_p = map_dbl(data, ~ agg_slope_p(., region_name, 100)))
```
...
slope_p = function(bd, n_iter = 999, method = bisquare) {
  slope = function(permute) {
    bd %>%
      distance_fit_data(permute = permute) %>%
      method(y ~ x, data = .) %>%
      coef() %>%
      extract('x')
  }
  
  base_slope = slope(FALSE)
  perm_slopes = replicate(n_iter, slope(TRUE))
  
  r = sum(perm_slopes > base_slope)
...
distance_fit = function(dataset, lst) {
  data_frame(
    ...
    model_lm = map(data, ~ lm(y ~ x, data = .)),
    inter_lm = map_dbl(model_lm, ~ coef(.)['(Intercept)']),
    slope_lm = map_dbl(model_lm, ~ coef(.)['x']),
    p_lm = map_dbl(model_lm, ~ coef(summary(.))['x', 'Pr(>|t|)']),
    model_bs = map(data, ~ bisquare(y ~ x, data = .)),
    inter_bs = map_dbl(model_bs, ~ coef(.)['(Intercept)']),
    slope_bs = map_dbl(model_bs, ~ coef(.)['x']),
    p_value = map_dbl(lst, slope_p),
    p2 = map_dbl(model_bs, bs_model_p)
  )
}
...
distance_fit_results %>%
  ...
  select(dataset, bugdrug, inter_lm, slope_lm, p_lm, inter_bs, slope_bs, p_value, fdr) %>%
  mutate_at(vars(starts_with('slope')), ~ . * 1e3) %>%
  mutate_if(is.numeric, ~ round(., 3)) %>%
  arrange(dataset, bugdrug) %>%
  kable(caption = 'Point estimates of intercept (dr/du, in p.p. resistance per unit use) and slope (dr/du per 10^3 km).')
```
