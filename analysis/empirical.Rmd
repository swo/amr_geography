...
marketscan_maup = try_load('results/marketscan_maup.rds', {
  crossing(
    ...
    mutate(
      ...
      within_models = map2(bugdrug, group, ~ group_models(marketscan_maup_data[[.x]]$data, .y))
    )
}, force_run = TRUE)
```

```{r}
qf = function(xs, y) ecdf(c(xs, y))(y)

marketscan_maup %>%
  group_by(bugdrug) %>%
  filter(k != max(k)) %>%
  ungroup() %>%
  mutate(within_slopes = map(within_models, function(ms) { map_dbl(ms, function(m) coef(m)['did']) }),
         slope_q = map2_dbl(within_slopes, slope, qf),
         slope_n = map2_int(within_slopes, slope, function(xs, y) sum(xs < y, na.rm = TRUE))) %>%
  count(bugdrug, k, group_type, slope_n) %>%
  ggplot(aes(factor(slope_n), n, fill = group_type)) +
  facet_grid(bugdrug ~ k, scales = 'free_x') +
  geom_col(position = 'dodge') +
  xlab('cross slope greater than X within slopes') +
  ylab('no. iterations')

marketscan_maup %>%
  mutate(within_slopes = map(within_models, function(ms) { map_dbl(ms, function(m) coef(m)['did']) }),
         slope_diff = map2(within_slopes, slope, ~ .y - .x)) %>%
  select(bugdrug, k, group_type, slope_diff) %>%
  unnest() %>%
  ggplot(aes(factor(k), slope_diff, color = group_type)) +
  facet_wrap(~ bugdrug) +
  geom_boxplot() +
  ylim(-10, 10)        
  # ggplot(aes(factor(k), slope_q, color = group_type)) +
  # facet_wrap(~ bugdrug) +
  # geom_boxplot()
```

There are multiple metrics I could use, but they all seem to tell the same story. Here I'm showing:

- linear regression slope
- log odds ratio from a logistic regression
- Spearman correlation

```{r}
marketscan_maup %>%
  group_by(bugdrug, k, group_type) %>%
  summarize(med_slope = median(slope),
            cil = quantile(slope, 0.25),
            ciu = quantile(slope, 0.75)) %>%
  ungroup() %>%
  ggplot(aes(factor(k), med_slope, color = group_type)) +
  facet_wrap(~ bugdrug, scales = 'free_x') +
  geom_point() +
  geom_errorbar(aes(ymin = cil, ymax = ciu)) +
  xlab('no. aggregates') +
  ylab('use-resistance slope')
```

I think the Spearman correlation is easiest to visualize because it has the limited range.
...
marketscan_maup %>%
  ggplot(aes(factor(k), cor, color = group_type)) +
  facet_wrap(~ bugdrug, scales = 'free') +
  geom_violin() +
  ggtitle('Correlations, violin')

marketscan_maup %>%
  group_by(k, group_type, bugdrug) %>%
  summarize(mean = mean(cor),
            cil = quantile(cor, 0.25),
            ciu = quantile(cor, 0.75)) %>%
