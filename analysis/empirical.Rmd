...
unit_data <- named_bind_rows(marketscan, nhsn, europe) %>%
  ...
  select(label, dataset, bugdrug, unit, f_resistant, use, density, temperature, income) %>%
  nest(-label, -dataset, -bugdrug)
```
...
unit_data %>%
  ...
  xlab('Treatments per person per year') +
  scale_y_continuous(
    name = 'Isolates resistant (%)'
  ) +
  theme_cowplot() +
  theme(
