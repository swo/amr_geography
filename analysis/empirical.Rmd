...
marketscan = marketscan_use %>%
  ...
  select(unit_id, unit, bugdrug, use, n_resistant, n_isolates, population, density)
```
...
europe_units = read_tsv('../db/europe_units.tsv') %>%
  mutate(density = population / area) %>%
  select(unit_id, unit, long = longitude, lat = latitude, population, eu_region = region, density)
...
europe = read_tsv('../data/ecdc/data.tsv') %>%
  ...
  select(unit_id, unit, bugdrug, use, n_resistant = n_ns, n_isolates, population, density)
...
regression_p = function(bd, n_iter = 999, method = bisquare, max_dist = Inf) {
  vals = function(permute) {
    ...
    arru_density = method(arru ~ d + d_density, data = dfd) %>% coef1('d')
    loru_density = method(loru ~ d + d_density, data = dfd) %>% coef1('d')
...
distance_fit = function(dataset, lst) {
  data_frame(
    ...
    model_arru_density = map(data, ~ bisquare(arru ~ d + d_density, data = .)),
    model_loru_density = map(data, ~ bisquare(loru ~ d + d_density, data = .)),
    arru_p_density = map_dbl(p_values, ~ .['arru']),
    loru_p_density = map_dbl(p_values, ~ .['loru'])
  )
}
