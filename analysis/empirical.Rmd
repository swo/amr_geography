...
state_census = read_tsv('../db/census/state_census.tsv') %>%
  ...
  left_join(census_regions, by=c('state'='state_abbreviation'))

# swo: what does "ur" stand for? use-resistance?
maup_ur2 = read_tsv('../data/nhsn-ims/data.tsv') %>%
  # just exclude DC for now
  filter(state != 'DC') %>%
  mutate(did = us_europe_convert('quinolone', rx_1k_year)) %>%
  left_join(state_census, by='state') %>%
  rename(state_abbreviation = state, state = state.y)

maup_lm2 = function (df) {
  df %>%
    summarize(did = weighted.mean(did, w=population),
              pct_res = weighted.mean(pct_res, w=n_isolates)) %>%
    do(tidy(lm(pct_res ~ did, data=.), conf.int=TRUE)) %>%
    ungroup()
}

maup_region2 = maup_ur2 %>%
  group_by(region) %>%
  maup_lm2()

maup_division2 = maup_ur2 %>%
  group_by(division) %>%
  maup_lm2()

maup_state2 = maup_ur2 %>%
  group_by(state) %>%
  maup_lm2()

group_lm2 = function(ur, n_groups) {
  n_state = length(unique(ur$state))
  group = group_sample_idx(n_state, n_groups)
...
group_lm_simulation2 = function(ur, n_groups, n_iterations) {
  lapply(1:n_iterations, function(i) group_lm2(ur, n_groups)) %>%
    bind_rows() %>%
    filter(term == 'did') %>%
    mutate(n_groups = n_groups)
}

maup_group2 = lapply(list_n_groups, function(k) group_lm_simulation2(maup_ur2, k, n_iterations)) %>%
  bind_rows()
```
