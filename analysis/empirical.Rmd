...
adjacency_boot_f = function(df, model_f, stat_f, n_trials = 1e2, alpha = 0.05) {
...
lor_adjacency_results <- unit_data %>%
  nest(-dataset, -bugdrug) %>%
  mutate(
    results = map(data, ~ adjacency_boot_f(
      .,
...
adjacency_covariates_results <- unit_data %>%
  nest(-dataset, -bugdrug) %>%
  mutate(
    results = map(data, ~ adjacency_boot_f(
      .,
...
commute_model_f <- safely(function(data) {
  bisquare(dr_du ~ f_commuting, data = data)
      ...
      #function(data) bisquare(dr_du ~ f_commuting + d_income + d_density + d_temperature, data = data),
