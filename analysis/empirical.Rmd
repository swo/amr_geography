...
europe_units = read_tsv('../db/europe_units.tsv') %>%
  select(unit_id, unit, long = longitude, lat = latitude, population, eu_region = region)
...
regression_p = function(bd, n_iter = 999, method = bisquare, max_dist = Inf) {
  vals = function(permute) {
    ...
    arru_density = method(arru ~ d + d_density, data = dfd) %>% coef('d')
    loru_density = method(loru ~ d + d_density, data = dfd) %>% coef('d')
