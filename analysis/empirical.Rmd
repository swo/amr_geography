...
mapmap <- function(list_of_tibbles, f) {
  map(list_of_tibbles, function(df) {
    map(df, f)
  })
}

mapmap_dbl <- function(list_of_tibbles, f) {
  map(list_of_tibbles, function(df) {
    map_dbl(df, f)
  })
}
...
commuting_hist_data <- cross_data %>%
  select(cross_data) %>%
  unnest() %>%
  recode(dataset)
  ...
  # geom_histogram(aes(fill = adjacent)) +
  # scale_x_continuous('Commuting fraction (log 10)') +
  # ylab('Number of pairs of areas') +
  # theme_half_open() +
  # theme(strip.background = element_blank())
```

Many of the pairs had zero interactions and don't appear in this figure:
...
model_f1 <- function(df) rlm(dr_du ~ f_commuting, data = df)

results <- cross_data %>%
  mutate(
    model = map(cross_data, model_f1),
    slope = map_dbl(model, ~ coef(.)['f_commuting']),
    ratio_1pct = map_dbl(model, ~ coef(.)['f_commuting'] / coef(.)['(Intercept)'] * 0.01),
    l1o_model = mapmap(l1o_cross_data, model_f1)
  )

results %>%
  select(dataset, bugdrug, slope, ratio_1pct, l1o_model) %>%
  unnest() %>%
  mutate(
    l1o_slope = map_dbl(l1o_model, ~ coef(.)['f_commuting']),
    l1o_inter = map_dbl(l1o_model, ~ coef(.)['(Intercept)']),
    l1o_ratio_1pct = l1o_slope / l1o_inter * 0.01
  ) %>%
  group_by(dataset, bugdrug) %>%
  summarize(
    slope = unique(slope),
    ratio_1pct = unique(ratio_1pct),
    slope_se = jackknife.sd(l1o_slope),
    ratio_1pct_se = jackknife.sd(l1o_ratio_1pct)
  )
```

```{r}
median_diff <- function(df) median(df$dr_du[df$adjacent]) - median(df$dr_du[!df$adjacent])
mds <- function(df) median_diff(df) / median(df$dr_du)

results <- cross_data %>%
  mutate(
    md = map_dbl(cross_data, median_diff),
    ratio = map_dbl(cross_data, mds),
    l1o_md = mapmap_dbl(l1o_cross_data, median_diff),
    l1o_ratio = mapmap_dbl(l1o_cross_data, mds)
  )

results %>%
  select(dataset, bugdrug, md, ratio, l1o_md, l1o_ratio) %>%
  unnest() %>%
  group_by(dataset, bugdrug) %>%
  summarize(
    md = unique(md),
    md_se = jackknife.sd(l1o_md),
    ratio = unique(ratio),
    ratio_se = jackknife.sd(l1o_ratio)
  )
```

```{r}
model_f <- function(df) rlm(d_resistant ~ 0 + d_use + d_use * adjacent, data = df)
# model_f <- function(df) rlm(d_resistant ~ 0 + d_use + d_use * f_commuting, data = df)
slope_f <- function(model) coef(model)['d_use']
ratio_1pct_f <- function(model) coef(model)['d_use:adjacentTRUE'] / coef(model)['d_use'] * 0.01
# ratio_1pct_f <- function(model) coef(model)['d_use:f_commuting'] / coef(model)['d_use'] * 0.01

results <- cross_data %>%
  mutate(
    model = map(cross_data, model_f),
    slope = map_dbl(model, slope_f),
    ratio_1pct = map_dbl(model, ratio_1pct_f),
    l1o_model = mapmap(l1o_cross_data, model_f)
  )
...
results %>%
  ...
  unnest() %>%
  mutate(
    l1o_slope = map_dbl(l1o_model, slope_f),
    l1o_ratio_1pct = map_dbl(l1o_model, ratio_1pct_f)
  ) %>%
  group_by(dataset, bugdrug) %>%
  summarize(
    slope = unique(slope),
...
```{r}
model_f <- function(df) rlm(d_resistant ~ 0 + d_use + d_use * adjacent + d_temperature + d_income + d_density, data = df)
slope_f <- function(model) coef(model)['d_use']
ratio_1pct_f <- function(model) coef(model)['d_use:adjacentTRUE'] / coef(model)['d_use'] * 0.01
...
```{r}
model_f <- function(df) rlm(dr_du ~ adjacent + d_temperature + d_income + d_density, data = df)
#model_f <- function(df) rlm(dr_du ~ adjacent, data = df)
slope_f <- function(model) coef(model)['adjacentTRUE']
ratio_1pct_f <- function(model) coef(model)['adjacentTRUE'] / coef(model)['(Intercept)']
