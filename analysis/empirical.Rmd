...
canonical_grouping_plot = function(maup) {
  maup %>%
    ...
    geom_smooth(aes(linetype = canonical_type), method = 'lm', alpha = 0.1, color = 'black', size = 0.5) +
    scale_linetype_manual(values = c(5, 3, 1)) +
    geom_point(aes(shape = canonical_type)) +
    scale_shape_manual(values = c(16, 0, 1)) +
...
cgp = canonical_grouping_plot(marketscan_maup)
cgp

ggsave('fig/marketscan_canonical.pdf', plot = cgp, width = 180, height = 80, units = 'mm')
```

```{r marketscan_canonical_table}
...
distance_fit_data = function(bd, permute = FALSE) {
  ...
  crossing(unit1 = units, unit2 = units) %>%
    ...
    mutate(
      dist = map2_dbl(i1, i2, ~ bd$unit_distances[.x, .y]),
      dr = res2 - res1,
      du = use2 - use1,
      lor = log(odds(res2)) - log(odds(res1)),
      arru = dr / du,
      loru = lor / du,
      # swo: can modify this to get different intercepts
      d = dist,
...
regression_p = function(bd, n_iter = 9, method = bisquare, max_dist = Inf) {
  vals = function(permute) {
    dfd = distance_fit_data(bd, permute = permute) %>%
      filter(dist <= max_dist)
    
    arru = method(arru ~ d, data = dfd) %>% coef1('d')
    loru = method(loru ~ d, data = dfd) %>% coef1('d')
    arru_density = method(arru ~ d + d_density + d_income + d_temp, data = dfd) %>% coef1('d')
    loru_density = method(loru ~ d + d_density + d_income + d_temp, data = dfd) %>% coef1('d')
    
    c('arru' = arru, 'loru' = loru, 'arru_density' = arru_density, 'loru_density' = loru_density)
  }
...
distance_fit = function(dataset, lst) {
  data_frame(
    ...
    # doing analysis only when including pairs less than 1k distant
    data1k = map(data, ~ filter(., dist < 1e3)),
    model_arru_1k = map(data1k, ~ bisquare(arru ~ d, data = .)),
    model_loru_1k = map(data1k, ~ bisquare(loru ~ d, data = .)),
    #p_values1k = map(lst, ~ regression_p(., max_dist = 1e3)),
    #arru_p_1k = map_dbl(p_values1k, ~ .['arru']),
    # including population density
    model_arru_density = map(data, ~ bisquare(arru ~ d + d_density + d_income + d_temp, data = .)),
    model_loru_density = map(data, ~ bisquare(loru ~ d + d_density + d_income + d_temp, data = .)),
    arru_p_density = map_dbl(p_values, ~ .['arru_density']),
    loru_p_density = map_dbl(p_values, ~ .['loru_density'])
  )
}
...
distance_fit = function(dataset, lst) {
  data_frame(
    ...
    bugdrug = names(lst),
    data = map(lst, distance_fit_data),
    model_arru = map(data, ~ bisquare(dr ~ du + du:d, data = .)),
    model_loru = map(data, ~ bisquare(lor ~ du + du:d, data = .)),
    # doing analysis only when including pairs less than 1k distant
    data1k = map(data, ~ filter(., dist < 1e3)),
    model_arru_1k = map(data1k, ~ bisquare(dr ~ du + du:d, data = .)),
    model_loru_1k = map(data1k, ~ bisquare(lor ~ du + du:d, data = .)),
    # including population density
    model_arru_density = map(data, ~ bisquare(dr ~ du + du:d + d_density + d_income + d_temp, data = .)),
    model_loru_density = map(data, ~ bisquare(lor ~ du + du:d + d_density + d_income + d_temp, data = .))
  )
}
...
# eec = europe_maup_data$`Ec/q` %>%
#   regression_p(n_iter = 9999)
```
...
distance_fit_table = function(model, p) {
  ...
  distance_fit_results %>%
    ...
    spread(term, range) %>%
    rename(slope = d) %>%
    left_join(select(distance_fit_results, dataset, bugdrug, slope_p = !!p_), by = c('dataset', 'bugdrug')) %>%
    mutate(fdr = p.adjust(slope_p, 'BH') < 0.05) %>%
    arrange(dataset, bugdrug)
}

distance_fit_table(model_arru, arru_p) %>%
  kable(caption = 'Point estimates of intercept (ARRU) and slope (ARRU per 10^3 km).')

distance_fit_table(model_loru, loru_p) %>%
  kable(caption = 'Point estimates of intercept (LORU) and slope (LORU per 10^3 km).')

distance_fit_table(model_arru_1k, arru_p_1k) %>%
  kable(caption = 'Point estimates of intercept (ARRU) and slope (LORU per 10^3 km), but with the analysis restricted to just those pairs at most 1,000 km distant.')
```
...
distance_fit_results %>%
  mutate(
    data_lo = map(data, ~ mutate(filter(., dist < breakpoint), where = 'lo')),
    data_hi = map(data, ~ mutate(filter(., dist > breakpoint), where = 'hi')),
    data_comb = map2(data_lo, data_hi, bind_rows)
  ) %>%
  select(dataset, bugdrug, data_comb) %>%
  unnest() %>%
  ggplot(aes(dist, loru, color = where)) +
  facet_wrap(~ dataset + bugdrug, scales = 'free') +
  geom_point(shape = 1) +
