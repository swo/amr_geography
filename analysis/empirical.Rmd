...
```{r adjacency_with_rlm_bisquare, eval = FALSE}
...
```{r log_commuting_analysis, eval = FALSE}
log_commuting_results <- analysis_f(
  function(df) {
    mutate(df, x = log10(f_commuting + 1e-6)) %>%
      rlm(dr_du ~ x, data = .)
  },
  function(model) coef(model)['x'] * 0.01,
  function(model) coef(model) %>% { .['x'] / .['(Intercept)'] * 0.01 }
)

show_results(
  log_commuting_results,
  'Robust regression on Δρ/Δτ ~ log10(C + 1e-6). "Coef" is β. "Ratio" is β / μ.'
)
```
...
adjacency_plot100 <- boxplot_f(cross_data, 1.0)
ggsave('fig/adjacency_plot100.pdf', plot = adjacency_plot100)
```
...
commute_plot <- cross_data %>%
  ...
  unnest() %>%
  #filter(!is.na(f_commuting)) %>%
  group_by(dataset) %>%
  mutate(x = log10(f_commuting + 1e-6)) %>%
