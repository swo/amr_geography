...
ms_maup_state = ms_maup_ur %>%
  maup_lm(c('bugdrug', 'geo_unit'), 'bugdrug')
...
state_distances = data_frame(long = state.center$x, lat = state.center$y) %>%
  dist() %>%
  as.matrix()

random_groups = function(geo_units, k, geo_unit_distances, geo_unit_names) {
  if (any(duplicated(geo_units))) stop('unique geo units required')
  
  stopifnot(length(geo_units) >= k)
  stopifnot(all(geo_units %in% geo_unit_names))
  
  geo_unit_idx = match(geo_units, geo_unit_names)
  
  samples = sample1(length(geo_unit_idx), k)
  
  zone_centers = sample(geo_unit_idx, k)
  zones = geo_unit_distances[geo_unit_idx, zone_centers] %>%
    apply(1, which.min) %>%
    { geo_unit_names[zone_centers[.]] }
  
  data_frame(geo_unit = geo_units,
             sample = samples,
             zone = zones)
}

group_lm = function(ur, k, geo_unit_distances=state_distances, geo_unit_names=state.name) {
  groups = random_groups(unique(ur$geo_unit), k, geo_unit_distances, geo_unit_names)
  
  sample_lm = ur %>%
    left_join(groups, by = 'geo_unit') %>%
    group_by(sample) %>%
    maup_lm(c('bugdrug', 'sample'), 'bugdrug') %>%
    mutate(type = 'sample')
  ...
  bind_rows(sample_lm, zone_lm)
}

ks = c(4, 9, 15, 25, 40)
n_iterations = 100

ms_maup_results = crossing(k=ks, iteration=1:n_iterations) %>%
  group_by(k, iteration) %>%
  do(group_lm(ms_maup_ur, .$k, state_distances, state.name)) %>%
  ungroup()
```
...
ms_maup_results %>%
  bind_rows(
    ms_maup_state %>% mutate(type = 'actual', k = 44),
    ms_maup_division %>% mutate(type = 'actual', k = 9),
    ms_maup_region %>% mutate(type = 'actual', k = 4)
  ) %>%
  filter(term == 'did') %>%
  ...
  ggplot(aes(factor(k), mean_slope)) +
...
nhsn_maup_results = crossing(k=c(ks, 45), iteration=1:n_iterations) %>%
  group_by(k, iteration) %>%
...
nhsn_maup_results %>%
  bind_rows(
    nhsn_maup_state %>% mutate(type = 'actual'),
    nhsn_maup_division %>% mutate(type = 'actual'),
    nhsn_maup_region %>% mutate(type = 'actual')
  ) %>%
  filter(term == 'did') %>%
  group_by(type, k) %>%
  summarize(mean_slope = mean(estimate),
...
```{r}
europe_centers = read_tsv('../db/europe_centers/europe_centers.tsv')

europe_maup_ur = read_tsv('../data/ecdc/data.tsv') %>%
  mutate(bugdrug = case_when(
  ...
  rename(geo_unit = country, n_resistant = n_ns)

europe_distances = europe_centers %>%
  select(latitude, longitude) %>%
  dist() %>%
  as.matrix()

europe_names = europe_centers$country

europe_maup_country = europe_maup_ur %>%
  maup_lm(c('bugdrug', 'geo_unit'), 'bugdrug')

europe_maup_results = crossing(k=c(5, 10, 15, 20, 25), iteration=1:n_iterations) %>%
  group_by(k, iteration) %>%
  do(group_lm(europe_maup_ur, .$k, europe_distances, europe_names)) %>%
  ungroup()
```

```{r}
europe_maup_results %>%
  bind_rows(europe_maup_country %>% mutate(type = 'actual', k = 26)) %>%
  filter(term == 'did') %>%
  group_by(bugdrug, type, k) %>%
  summarize(mean_slope = mean(estimate),
            p25 = quantile(estimate, 0.25),
