...
distance_fit_results %>%
  mutate(
    ...
    arru_dud = map_chr(model_arru, ~ peci(., 'du:d', mult = 1e4)),
    arru_ratio = map_dbl(model_arru, ~ round(coef2(.) * 1e4, 2)),
    loru_du = map_chr(model_loru, ~ peci(., 'du')),
    loru_dud = map_chr(model_loru, ~ peci(., 'du:d', mult = 1e4)),
    loru_ratio = map_dbl(model_loru, ~ round(coef2(.) * 1e4, 2)),
  ) %>%
  select(
    dataset, bugdrug,
