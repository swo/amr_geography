...
random_groups = function(states, k) {
  if (any(duplicated(states))) stop('unique states required')
  
  stopifnot(length(states) >= k)
  stopifnot(all(states %in% state.name))
  
  state_idx = match(states, state.name)
  
  samples = sample1(length(state_idx), k)
  
  zone_centers = sample(state_idx, k)
  zones = state_distances[state_idx, zone_centers] %>%
    apply(1, which.min) %>%
    { state.name[zone_centers[.]] }
...
group_lm = function(ur, k) {
  groups = random_groups(unique(ur$state), k)
  
  sample_lm = ur %>%
    left_join(groups, by = 'state') %>%
    group_by(sample) %>%
