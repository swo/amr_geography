...
random_samples = function(k) {
  stopifnot(n_units >= k)
  data_frame(unit = unit_data$unit,
             group = as.character(sample1(n_units, k)))
}
...
marketscan_maup = crossing(
  k = ks,
  iteration = 1:n_iterations,
  type = c('sample', 'zone')
) %>%
  mutate(group = map2(type, k, group_f)) %>%
  bind_rows(state_grouping, division_grouping, region_grouping) %>%
  crossing(bugdrug = c('Ec/q', 'Sp/bl', 'Sp/m')) %>%
