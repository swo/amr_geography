...
x2_plot_f = function(df) {
  ...
  scale_x_discrete(name = expression('amount of interaction ' (epsilon))) +
  scale_y_continuous(
    name = 'resistance (% of hosts)',
    labels = scales::percent_format(accuracy = 1, suffix = ''),
    limits = c(0, 1.0),
    expand = c(0, 0),
    sec.axis = sec_axis(trans = ~ ., name = expression('intervention magnitude ' (Delta * tau)), breaks = NULL)
  ) +
  theme_classic() +
  theme(
