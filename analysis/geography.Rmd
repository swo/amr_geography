...
whn1 %>%
  mutate(linear_model = map(results, ~ lm(rho ~ tau, data = .)),
         ...
         cil = map_dbl(linear_model, ~ confint(.)['tau', 1]),
         ciu = map_dbl(linear_model, ~ confint(.)['tau', 2])) %>%
  mutate_at(vars(slope, cil, ciu), ~ round(. * 100)) %>%
  mutate(slope_range = sprintf('%.1i (%.1i to %.1i)', slope, cil, ciu)) %>%
  select(n_pop, epsilon, slope_range) %>%
  spread(n_pop, slope_range) %>%
  kable()
```
...
nest_results %>%
  ...
  write_csv('results/whn_nest_results.csv') %>%
  kable()
```
