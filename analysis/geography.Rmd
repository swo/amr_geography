...
In the D-types model, there are $n_D$ "duration"-types, each with clearance rate $\mu^{(d)}$. Each type $d$ has sensitive and resistant strains, so the compartments are uncolonized $X$, sensitive-colonized $S^{(d)}$, and $R^{(d)}$. The D-types are subject to balancing selection, encoded by
$$
v^{(d)} = \left( 1 - \left[ \frac{S^{(d)} + R^{(d)}}{\sum_{d=1}^{n_D} (S^{(d)} + R^{(d)})} - \frac{1}{n_D} \right]\right)^k,
$$
where $k \geq 1$ is the force of that selection. Thus:
$$
\begin{aligned}
\dot{S}^{(d)} &= v^{(d)} \beta S^{(d)} X - (\tau + \mu^{(d)}) S^{(d)} \\
\dot{R}^{(d)} &= v^{(d)} \frac{\beta}{c_\beta} R^{(d)} X - c_\mu \mu^{(d)} R^{(d)} \\
\end{aligned}
$$
...
$$
\rho = \frac{\sum_d R^{(d)}}{\sum_d \left( S^{(d)} + R^{(d)} \right)}.
$$

#### Populations model

To extend this to multiple populations, we give each compartment a subscript $i$ and then need to be careful about the tranmission terms:
$$
\begin{aligned}
\dot{S}_i^{(d)} &= v_i^{(d)} \sum_j \beta_{i \leftarrow j} \frac{S_j^{(d)}}{N_j} X_i - \left( \tau_i + \mu^{(d)} \right) S_i^{(d)} \\
\dot{R}_i^{(d)} &= v_i^{(d)} \frac{1}{c_\beta} \sum_j \beta_{i \leftarrow j} \frac{R_j^{(d)}}{N_j} X_i - c_\mu \mu^{(d)} R_i^{(d)} \\
\end{aligned}
$$
...
dtypes_base_parms$mud = with(dtypes_base_parms,
...
```{r dtype_cross}
# pack the array into a vector
dtypes_pack = as.vector

# unpack the vector into an array
dtypes_unpack = function(x, n_d, n_pop) {
  array(x, dim = c(n_d, 3, n_pop),
        dimnames = c(dtype = NULL, phenotype = c('X', 'R', 'S'), population = NULL))
}

# for initially packing the easy-to-understand data frame into an array
dtypes_df_to_array = function(df) {
  # check we have the right column names
  stopifnot(setequal(names(df), c('population', 'dtype', 'phenotype', 'value')))
  ...
  # make sure all the rows are in order
  df %<>% arrange(population, phenotype, dtype)

  # initialize the array
  # note that R uses column-major ordering for arrays (leftmost index varies most quickly),
  # so if the data frame is ordered by pop, pheno, D, then array must have indices in the
  # order D, pheno, pop. Phenotypes R and S are in alphabetical order.
  output = dtypes_unpack(NA, n_d, n_pop)

  # assign the X values just to the first "D-type"
  output[1, 'X', ] <- df %>% filter(phenotype == 'X') %$% value

  # assign the S and R values
  output[, c('R', 'S'), ] <- df %>% filter(phenotype %in% c('R', 'S')) %$% value

  output
}

# for finally unpacking the array into a data frame
dtypes_array_to_df = function(arr) {
  arr %>%
    as_tibble() %>%
    mutate(population = 1:n()) %>%
    gather('key', 'value', -population) %>%
    separate(key, c('phenotype', 'dtype'))
}

dtypes_ode_func = function(t, state_vector, parms) {
  state_array = dtypes_unpack(state_vector)

  # X is a vector of length n_pop
  X = state_array[1, 'X', ]
  # S and R are (n_d x n_pop) matrices
  S = state_array[, 'S', ]
  R = state_array[, 'R', ]
