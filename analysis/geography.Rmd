...
A <- tibble(x = c(-0.5, 0.5), y = c(-1, 1)) %>%
  ggplot(aes(x, y)) +
  geom_blank() +
  annotate("point", 0, 0, color = "red") +
  annotate("point", -1, -1) +
  annotate("point", 1, 1) +
...
A <- ggdraw() +
  draw_grob(grid::circleGrob(gp = grid::gpar(fill = "black")), scale = 0.5) +
  draw_grob(x = -0.6, y = 0, grid::textGrob("foo", gp = grid::gpar(col = "red")))

B <- whn2_plot1 +
  theme(plot.background = element_rect(color = "red"))

patch <- (A | B | whn2_plot2) /
  (whn2_plot3 | whn2_plot4) +
