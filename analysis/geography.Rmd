...
As a variation on the previous, imagine that the islands can interact with one another in a tunable way. First, arrange the $N$ islands in a ring. The interaction between two islands $i$ and $j$ will be a function of the distance between them $d(i, j) = \min( |i-j|, N - |i-j| )$ as well as a separation parameter $\alpha$. For $\alpha \to \infty$, the islands are completely separate; for $\alpha = 0$, the islands interact fully. Terms like $\beta S X$ in the original model become like:
$$
\beta \left( \sum_{j=1}^N W_{ij} S_j \right) X_i,
$$
where $\sum_j W_{ij} = 1$. For exponential decay in interaction, this is:
$$
W_{ij} = \frac{ \exp\{ -\alpha \, d(i,j) \} }{\sum_{j=1}^N \exp \{ -\alpha \, d(i,j) \} }.
$$
Alternatively, we can parameterize this using the distance $d$:
$$
W(d) = \frac{e^{-\alpha d}}{\sum_{d'=0}^{\lfloor n/2 \rfloor} e^{-\alpha d'} }
$$

For $\alpha = 0$, $W_{ij} = 1/N$ for all $i$ and $j$. For $\alpha \to \infty$, $W_{ij} = \mathbf{1}_{i=j}$. So, all told, we write:
$$
S_i' = \frac{ \sum_j W_{ij} S_j }{ \phi_i }
$$
and similarly for $R_i'$ and $D_i'$.

Use can be distributed randomly across the ring, or there can be some autocorrelation, say via a sinusoid.
...
ring_dist = function(i, j, n) min(abs(i - j), n - abs(i - j))

ring_ode_func = function(t, state_vector, parms) {
  state = ring_v2l(state_vector)
  
  with(c(state, parms), {
    n = length(phii)
    raw_weights = sapply(1:
    weight = function(i, j) exp(-alpha * ring_dist(i, j, n)) / sum(exp(-alpha * 
    raw_weights = outer(1:n, 1:n, FUN=function(i, j) exp(
