...
- Introduce chain link idea. We expect that chain link fences would increase the measured slopes. by how much? how much do we worry about the fact that we don't? Introduce the existing use/res models in terms of that.
- Most groups are untreated in Ethiopia, so you expect that resistance in treated groups is below what it would be otherwise
- Azithro resistance is all bystander in Ethiopia
- Need a better transition into the nested model. People can get the mixing thing, but the question about the nested is more difficult.
- Did you do a good grouping?
    - eg what if nearby places have really different antibiotic use? this is why we think that it doesn't matter a lot if you live in this house or the one next door
    - but it does make a difference at a population level about France vs. Netherlands
    - but only talk about states because people are used to those aggregations
- What if relationships isn't linear?
- Introduce noise into x and y measurements. Maybe only measure correlation? (That's confusing when you have no noise.)
- Need a visualization for what happens to confidence intervals on slope if you do the aggregation
- What if you use a more correct modeling of use and resistance?
- Geography is only one way of doing the population. Sex as a bad grouper b/c men and women interact a lot, but MSM/MSW as a good grouper. Race as an example of one that might make more sense.
- Put stats on claims that mean slope goes up or down with scale. Use that chi-square-like test?
- Cite Lipsitch & Samore for the thing about population-level being different from individual-level
- Cite Mossong for contact stratification
- "equilibration" means that every place has the access to the same pathogen phenotypes. this does not account for temporal possibilities. just address up front that temporal perturbations are one way of looking at this problem.
...
There are $n$ populations each of size $N_i$. Changes in population membership are slow compared to the transmission dynamics, so each $N_i$ remains fixed.

In a first simulation, the populations are all transmission populations. A parameter $\epsilon \in [0, 1]$ controls the relative proportion of contacts within versus between the transmission populations. $\epsilon = 0$ means that all contacts are within a population; $\epsilon = 1$ means that contacts are equally likely within and across populations. Let $\beta'_{i \leftarrow j}$ be the the proportion of contacts for individuals in $i$ that are in $j$ so that $\beta_{i \leftarrow j} = \beta \beta'_{i \leftarrow j}$, where $\beta$ is a constant. We require that $\sum_j \beta'_{i \leftarrow j} = 1$.
$$
\begin{aligned}
...
The parameter values are drawn from the paper: $n = 16$, $\beta = 2$, $c_\beta = 1$, $c_\mu = 1.1$, $k = 15$, $\tau \in [0.04, 0.20]$, $\mu_x$ evenly spaced in $[0.5, 2]$. The original figure uses $\tau = 0.075$.
...
nest_results %>%
  ...
  mutate(super_model = map(data, ~ lm(super_rho ~ super_tau, data = .)),
         ...
         mean_sub_slope = map_dbl(data, ~ mean(.$sub_slope)),
         slope_ratio = mean_sub_slope / super_slope) %>%
  select(eps_super, eps_sub, super_slope, mean_sub_slope, slope_ratio) %>%
...
It also seems like aggregating slightly increases the point estimate.

```{r steps}
steps = nest_results %>%
  unnest() %>%
  filter(eps_super == 0.0, eps_sub == 0.5)

steps_aggregated = steps %>%
  group_by(superpop) %>%
  summarize_at(vars(tau, rho), mean)

geom_lm = partial(geom_smooth, method = 'lm', se = FALSE)

ggplot(NULL, aes(tau, rho)) +
  geom_lm(data = steps, color = 'red') +
  geom_point(data = steps, aes(color = factor(superpop))) +
  geom_lm(data = steps_aggregated, color = 'black') +
  geom_point(data = steps_aggregated, size = 3) +
  guides(color = FALSE) +
  theme_classic() +
  xlab('courses per month') +
  ylab('proportion resistant') +
  ylim(0, 1)

nest_results %>%
  mutate(model = map(data, ~ lm(rho ~ tau, data = .)),
         slope = map_dbl(model, ~ coef(.)['tau']),
         slope_ciu = map_dbl(model, ~ confint(.)['tau', 1]),
         slope_cil = map_dbl(model, ~ confint(.)['tau', 2])) %>%
  ggplot(aes(factor(eps_super), slope, color = factor(eps_sub))) +
  geom_point() +
  geom_errorbar(aes(ymin = slope_cil, ymax = slope_ciu))
```
...
Say that there are $n_0$ points, and we think we aggregate them into $n$ points (i.e., $n_0 / n$ subpops per superpop). This decreases the variance in the residuals (from $\sigma_{\epsilon,0}^2$ to $\sigma_{\epsilon,0}^2 / (n_0 / n)$) but decreases the number of points and the variance in $x$:
$$
\begin{aligned}
\mathrm{Var}[\hat{\beta}_1] &= \frac{\sigma_\epsilon^2}{n \sigma_x^2} \\
  &= \frac{\sigma_{\epsilon,0}^2 / (n_0 / n)}{n \sigma_x^2} \\
  &= \frac{\sigma_{\epsilon,0}^2}{n_0 \sigma_x^2}.
\end{aligned}
$$
This is just like the estimator for the non-aggregated estimator except that it has the aggregate variance $\sigma_x^2$ rather than the original $\sigma_{x,0}^2$. So the variance in $\beta_1$ will grow like $\sigma_{x,0}^2 / \sigma_x^2$.
