...
dtypes_df_to_array = function(df) {
  ...
  # so to get the array to be [pop, pheno, dtype], need to order the data frame this way
  df %<>% arrange(dtype, phenotype, population)
  # ensure the ordering of the phenotypes
  df %<>% mutate(phenotype = factor(phenotype, levels = c('X', 'S', 'R')))

  # initialize the array
  output = dtypes_unpack(NA, n_d, n_pop)

  # assign the X values just to the first "D-type"
  output[, 'X', 1] <- df %>% filter(phenotype == 'X') %$% value
  # assign the S and R values
  output[, c('S', 'R'), ] <- df %>% filter(phenotype %in% c('S', 'R')) %$% value
...
dtypes_ode_func = function(t, state_vector, parms) {
  ...
  with(parms, {
    dS_id = v_id * (beta_ij %*% (S_id / N_i)) * X_i - (tau_i + mu_d) * S_id
    dR_id = v_id * (beta_ij %*% (R_id / N_i)) * X_i - c_mu * mu_d * R_id
    dX_i = -rowSums(dS_id + dR_id)

    out_array = dtypes_unpack(NA, n_d, n_pop)
    out_array[, 'X', 1] <- dX_i
    out_array[, 'X', -1] <- 0
    out_array[, 'S', ] <- dS_id
    out_array[, 'R', ] <- dR_id

    dtypes_pack(out_array)
  })
