...
whn2 %>%
  mutate(
    ...
    lor_rho = logodds(max_rho) - logodds(min_rho),
    dr_du = delta_rho / delta_tau
    #dr_du = lor_rho / delta_tau
  ) %>%
  filter(is.finite(dr_du)) %>%
  ggplot(aes(epsilon, dr_du, color = factor(delta_tau))) +
  geom_point() +
  geom_line() +
...
whn2_plot_data = whn2 %>%
  ...
  unnest() %>%
  filter(epsilon %in% c(0.05, 0.10, 0.50))
