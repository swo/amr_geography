...
```{r whn_nest_plot}
whn_nest_subset = whn_nest_results %>%
  unnest() %>%
  mutate(
    panel = case_when(
      eps_super == 0.0 & eps_sub == 0.0 ~ 'list(epsilon[super] == 0.0, epsilon[sub] == 0.0)',
      eps_super == 0.0 & eps_sub == 0.5 ~ 'list(epsilon[super] == 0.0, epsilon[sub] == 0.5)',
      eps_super == 0.5 & eps_sub == 0.5 ~ 'list(epsilon[super] == 0.5, epsilon[sub] == 0.5)',
      TRUE ~ 'other'
    )
  ) %>%
  filter(panel != 'other')

whn_nest_region = whn_nest_subset %>%
...
whn_nest_contrast = whn_nest_subset %>%
  nest(-panel) %>%
  mutate(data = map(data, cross_nest)) %>%
  unnest() %>%
  mutate(
    lor = log_odds_ratio(rho1, rho2),
    d_rho = rho1 - rho2,
    d_tau = tau1 - tau2,
...
panel3_text = data_frame(
  label = c('different super-pop.', 'same super-pop.'),
  adjacent = c(FALSE, TRUE),
  panel = 'list(epsilon[super] == 0.0, epsilon[sub] == 0.5)',
  x = c(0.12,  0.06),
  y = c(0.25, -0.10)
)

panel3 = whn_nest_contrast %>%
  ...
  xlab('difference in antibiotic use') +
  scale_y_continuous(
    'resistance difference (%)    ',
    labels = scales::percent_format(accuracy = 1, suffix = '')
  ) +
  guides(color = 'none') +
  geom_text(data = panel3_text, aes(x, y, label = label), size = 8 / ggplot2::.pt) +
  theme_classic() +
  theme(
    axis.title = element_text(size = 8),
    axis.text = element_text(size = 8, color = 'black'),
...
ggsave(
  'fig/whn_nest.pdf', whn_nest_plot,
  width = 190, height = 120, unit = 'mm'
)
```
...
```{r dtypes_nest_plot}
dtypes_nest_subset = dtypes_nest_results %>%
  unnest() %>%
  group_by(eps_super, eps_sub, superpop, pop, tau, phenotype) %>%
...
dtypes_nest_region = dtypes_nest_subset %>%
  group_by(panel, superpop) %>%
  summarize_at(c('tau', 'rho'), mean) %>%
  ungroup()

dtypes_nest_contrast = dtypes_nest_subset %>%
  nest(-panel) %>%
  mutate(data = map(data, cross_nest)) %>%
  unnest() %>%
  mutate(
...
dtypes_nest_plot = egg::ggarrange(
  panel1, panel2, panel3,
  nrow = 3,
  labels = letters[1:3]
)
...
ggsave(
  'fig/dtypes_nest.pdf', dtypes_nest_plot,
  width = 190, height = 120, unit = 'mm'
)
```
