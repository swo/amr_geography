...
whn_nest_plot = whn_nest_results %>%
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      trans = ~ .,
      # \varepsilon_\downarrow
      name = expression(bold(ε)['' %down% '']),
      breaks = NULL
    )
  ) +
  theme_classic() +
...
ggsave('fig/whn_nest.pdf', plot = whn_nest_plot, width = 88, height = 110, units = 'mm')
```
...
dtypes2 %>%
  ...
  mutate_if(is.numeric, ~ round(., 2)) %>%
  arrange(epsilon, delta_u) %>%
  kable(caption = '2-population effect sizes, D-types model')
...
ggsave('fig/dtypes_nest.pdf', plot = dtypes_nest_plot, width = 88, height = 110, units = 'mm')
```
