...
Imagine that $x$ and $y$ are linearly related:
$$
y_i = m x_i + b + \epsilon_i
$$
The variance $\sigma_y^2$ partitions:
$$
\sigma_y^2 = m^2 \sigma_x^2 + \sigma_\epsilon^2
$$
so that the correlation coefficient between $x$ and $y$, the fraction of $\sigma_y^2$ "explained" by $x$, is:
$$
\begin{aligned}
r^2 &\approx \frac{m^2 \sigma_x^2}{\sigma_y^2} \\
  &= \frac{m^2 \sigma_x^2}{m^2 \sigma_x^2 + \sigma_\epsilon^2} \\
  &= \frac{1}{1 + \sigma_\epsilon / (m^2 \sigma_x^2)} \\
  &= m^2 \frac{\sigma_x^2}{\sigma_y^2} + \mathcal{O}\left( \frac{\sigma_x^2}{\sigma_\epsilon^2} \right)^2 \text{ for } \sigma_x^2 \ll \sigma_\epsilon^2
\end{aligned}
$$
In other words, the observed correlation coefficient is a function of the slope as well as of the scale of the $x$-axis. If $x$ varies over a small range, $r^2$ will tend to increase linearly with the square of the standard deviation of $x$.

The correlation coefficient between them is, roughly speaking, the fraction of the variance $\sigma_y^2$ "explained" by $x$.
so that
...
ggplot(data=NULL, aes(tau, rho)) +
  ...
  geom_smooth(data=davies_within, color='gray', method='lm', se=FALSE) +
...
islets = crossing(island_tau = round(c(1.25, 1.50, 1.75) / 12, 3),
                  delta_tau = seq(-0.1, 0.1, length.out = 11),
                  k = c(1.0),
                  alpha = c(0.0, 0.1, 0.5, 1.0)) %>%
  mutate(tau = island_tau + delta_tau) %>%
  group_by(island_tau, alpha, k) %>%
...
comp1 = davies_cross %>%
  filter(k == 1) %>%
  crossing(island_tau = unique(islets$island_tau))
...
islets %>%
  ggplot(aes(tau, rho, color=factor(island_tau))) +
  geom_point() + geom_line() +
  geom_line(data=comp1, color='black') +
  geom_line(data=comp2, linetype=2) +
  facet_wrap(~ alpha)
```
...
ring_sim = function(taui, k, alpha, tau50=1.5/12) {
  n_islets = length(taui)
  phii = rep(1 / n_islets, n_islets)
...
rings = data_frame(on_island_f = c(0.99, 0.50, 0.25, 0.10, 0.05)) %>%
  ...
  group_by(on_island_f, alpha) %>%
  do(ring_sim(taui = ring_tau, k = 1.0, alpha = .$alpha)) %>%
  ungroup() %>%
  mutate(rho = R/(S + R + D))
```
...
# compare with within-host neutral cross-island model
# black lines
ring_comp1 = davies_cross %>%
  filter(k == 1, tau <= 3 / 12) %>%
  mutate(on_island_f = 1)

# time model
ring_comp2 = davies_time_comp %>%
  filter(k == 1, island_tau == 1.5 / 12) %>%
  mutate(on_island_f = NA)

rings %>%
  ggplot(aes(tau, rho, color = factor(on_island_f))) +
  geom_point() +
  geom_line() +
  geom_line(data = ring_comp1) +
  geom_line(data = ring_comp2)
```

The original within-island simulation arises from this as the number of (equally-populated) islands goes to infinity, all other islands have a fixed use, and islands are well-mixed.

### Weird uses

What if islands had randomly mixed uses? Like, there's no spatial autocorrelation: each use is drawn from a random uniform distribution. Or, we just mix up the previous ones.

```{r}
ring_tau2 = runif(n_ring, 0, 3.0 / 12)
ring_tau3 = sample(ring_tau)

rings2 = data_frame(on_island_f = c(0.99, 0.50, 0.25, 0.10, 0.05)) %>%
  mutate(alpha = f_to_alpha(on_island_f, n_ring)) %>%
  group_by(on_island_f, alpha) %>%
  do(ring_sim(taui = ring_tau2, k = 1.0, alpha = .$alpha)) %>%
  ungroup() %>%
  mutate(rho = R/(S + R + D))

rings3 = data_frame(on_island_f = c(0.99, 0.50, 0.25, 0.10, 0.05)) %>%
  mutate(alpha = f_to_alpha(on_island_f, n_ring)) %>%
  group_by(on_island_f, alpha) %>%
  do(ring_sim(taui = ring_tau3, k = 1.0, alpha = .$alpha)) %>%
  ungroup() %>%
  mutate(rho = R/(S + R + D))
