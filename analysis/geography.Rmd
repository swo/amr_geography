...
```{r}
# subpops have size = 1 / (szie of their superpop)
subpop_size_f = function(superpop) sapply(superpop, function(x) 1 / sum(superpop == x))
stopifnot(all(subpop_size_f(c(1, 2, rep(3, 3), 2, 5)) == c(1.0, 0.5, rep(1 / 3, 3), 0.5, 1.0)))

renumber = function(x) match(x, unique(x))

whn_nest_sim = function(taui, superpop, epsilon) {
  stopifnot(length(superpop) == length(taui))
  n_pop = length(taui)
  n_superpop = length(unique(superpop))
  
  # renumber superpop ids to 1, ..., no. superpops
  superpop = renumber(superpop)
  stopifnot(max(superpop) == n_superpop)
  
  superpop_size = 1 / n_superpop # assume equal pop sizes
  pop_size = subpop_size_f(superpop)
  
  # make the (unscaled) superpop contact matrix
  beta_super = matrix(epsilon / n_superpop, ncol = n_superpop, nrow = n_superpop)
  diag(beta_super) <- 1 - epsilon * (1 - 1 / n_superpop)
  
  # make the population contact matrix
  contact = outer(1:n_pop, 1:n_pop, Vectorize(function(i, j) {
    pop_size[j] * beta_super[superpop[i], superpop[j]]
  }))

  stopifnot(all(rowSums(contact) == 1.0))
  stopifnot(all(colSums(contact) == 1.0))
...
x = whn_nest_sim(taui = rep(seq(0.05, 0.20, length.out = 4), each = 4) + 0.01 * rep(seq(-1, 1, length.out = 4), 4),
                 superpop = rep(1:4, each = 4),
                 epsilon = 0.0)

ggplot(x, aes(tau, rho, color = factor(superpop))) +
  geom_point() +
  geom_line() +
  ylim(0, 1)
```
