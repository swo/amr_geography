...
eu_names <- with(eu_flights, {
  intersect(unique(country1), unique(country2))
}) %>%
  sort()
...
eu_flights_matrix <- eu_flights %>%
  filter(country1 %in% eu_names, country2 %in% eu_names) %>%
  arrange(country1, country2) %>%
  spread(country2, n, fill = 0) %T>%
  {
...
grow_diagonal <- function(mat, f) `diag<-`(mat, diag(mat) * 1 / f)

eu_matrix_f <- function(f) {
  eu_flights_matrix %>%
    symmetrize() %>%
    grow_diagonal(f) %>%
    scale_rows()
...
results <- tibble(internal_f = c(0.1, 0.01, 0.001)) %>%
  mutate(parms = map(internal_f, ~ `$<-`(null_parms, "transmission_matrix", eu_matrix_f(.)))) %>%
  bind_rows(tibble(internal_f = NA, parms = list(null_parms))) %>%
  mutate(sim = map(parms, whn_sim))
```

```{r}
results %>%
  select(internal_f, sim) %>%
  unnest() %>%
...
results %>%
  ...
  select(internal_f, slope, reduction) %>%
  arrange(reduction)
```
